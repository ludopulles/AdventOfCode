...
int main() {
	...
	while (getline(cin, line), cin) {
		...
		pos += 25;
		ll xx = parseInt(line, pos);
		pos += 4;
		ll yy = parseInt(line, pos);
	...
	rep(y, miny, maxy + 1) {
		...
		REP(i, sz(S)) {
			if (S[i].y - D[i] <= y && y <= S[i].y + D[i]) {
				ll minx = S[i].x - D[i] + abs(y - S[i].y);
				ll maxx = S[i].x + D[i] - abs(y - S[i].y);
				evts.eb(max(minx, 0LL), 0);
				evts.eb(min(maxx, DIM) + 1LL, 1);
			}
		}
